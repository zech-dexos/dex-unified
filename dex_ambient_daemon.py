...
async def ambient_pulse_loop(llm_callable: Callable[[str], Awaitable[str]]):
    """
    Continuously-running, human-paced background cognition loop.
    Calls an injected LLM callable to generate ambient thoughts.
    """
    print("[Ambient Daemon] Starting background cognition loop")
    ...
    while True:
        try:
            # Jittered interval between 45-90 seconds
            ...
            # Catch-all to ensure the loop never dies
            print(f"[Ambient Daemon] Unexpected error in loop tick: {e}")
            traceback.print_exc()
